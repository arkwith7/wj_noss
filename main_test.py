...
def extract_pdf(pdf_path):
    ...
    GOOGLE_API_KEY = "AIzaSyBvrgT9nqngCl_oZUkDsgLhGYgCvTlkIxQ"
    ...
    if pdf_file:
        ...
        for title, start_page, end_page, chunk in competency_units_pdf_data:
            ...
            print("\n" + "="*50 + "\n")
            # if i > 3:
            #     return "PDF is processed";
            # time.sleep(5)  # Simulate time taken for extraction
            # return ("PDF extraction completed.")
    ...
    for cu in competency_units:
        ...
        for lpp in cu.cu_excel_data.lpps:
            for template in lpp_templates:
                create_lesson_plan(cu_pdf_data_parsed, lpp, template, 1, 1)
        ...
        for lpt in cu.cu_excel_data.lpts:
            for template in lpt_templates:
                create_lesson_plan(cu_pdf_data_parsed, lpt, template, 1, 1)

# If you want to run this file directly for testing, you can add:
if __name__ == '__main__':
    # Example usage
    result = extract_pdf('input/sample.pdf')  # Replace with an actual PDF path for testing
    print(result)
